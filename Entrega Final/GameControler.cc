...
GameControler::GameControler(int w, int h, int f) :
   gameOver(false),
   playerLives(3),
{
...
// UPDATE MAIN
//-----------------------------
void GameControler::update(double dt) {
   // background
   bg->update(dt);
   // player
   if (player) {
...
// INPUT MAIN
//--------------------------------
void GameControler::input(ALLEGRO_KEYBOARD_STATE& kb) {
   if (player) {
...
// its duration
bool GameControler::is_game_over() {
...
void GameControler::CircleLaser(std::shared_ptr<Enemy> E) {
...
void GameControler::bossFire(std::shared_ptr<Enemy> e){
   //if (!bossFirstShot) {
   //      for(int i = 800; i <= 1000; i += 50) {
...
// UPDATE FUNCTIONS
//=================================
void GameControler::spawnBoss() {
   addBoss(Point(850, 300), al_map_rgb(155, 0, 0), Vector(-50, 0));
   bossExists = true;

}

void GameControler::addBoss(const Point& cen, const ALLEGRO_COLOR& col, const Vector& spd) {
   enem.push_back(std::make_shared<Boss> (cen, col, spd));
}

void GameControler::updateBoss() {
   // spawn condition
...
void GameControler::drawBossIncomingMessage() {
...
void GameControler::updateHighscores() {
...
void GameControler::updateScore(ALLEGRO_COLOR& c) {
...
void GameControler::updateProjectilePosition(double dt) {
   if (!proj.empty()) {
...
void GameControler::updateEnemyPosition(double dt) {
   if (!enem.empty()) {
...
void GameControler::checkCollidingEnemyWithPlayer() {
...
void GameControler::checkCollisionOnPlayer() {
...
void GameControler::clean() {
   cullPlayer();
   cullProjectiles();
   cullEnemies();
}

void GameControler::cullPlayer() {
...
void GameControler::cullProjectiles() {
...
void GameControler::cullEnemies() {
...
void GameControler::respawnPlayer() {
...
// DRAW FUNCTIONS
//=================================
void GameControler::drawProjectiles() {
   if (!proj.empty()) {
      for (auto it = proj.begin(); it != proj.end(); ++it) { 
	 (*it)->draw();
      }
...
void GameControler::drawEnemies() {
   if (!enem.empty()) {
      for (auto it = enem.begin(); it != enem.end(); ++it) {
...
void GameControler::drawWeaponUpgradeStatus() {   
...
void GameControler::showGameOverMessage() {
...
// then y = 150, 300, 450
void GameControler::drawHighScores() {
   int yPosition = 150;
   for (int i = 0; i < 3; i++) {
...
void GameControler::addPlayerLaserTripleShot() {
...
void GameControler::addPlayerMissileSingleShot() {
...
// its width s2
bool GameControler::isPointBoxCollision(const Point& p1, const Point& p2, 
				 const int& s2) {
   return ((p1.x > p2.x - s2) &&
	   (p1.x < p2.x + s2) &&
	   (p1.y > p2.y - s2) &&
...
bool GameControler::doColorsMatch(const ALLEGRO_COLOR& a, const ALLEGRO_COLOR& b) {
   return (a.r == b.r && a.g == b.g && a.b == b.b);
}


bool GameControler::doHitboxesIntersect(const Point& centre1, const int& size1,
